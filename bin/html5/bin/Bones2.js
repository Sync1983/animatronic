...
ApplicationMain.main = function() {
	...
	nme.Lib.get_current().loaderInfo = browser.display.LoaderInfo.create(null);
	ApplicationMain.preloader = new NMEPreloader();
	nme.Lib.get_current().addChild(ApplicationMain.preloader);
	ApplicationMain.preloader.onInit();
	var urlLoader = new browser.net.URLLoader();
	urlLoader.set_dataFormat(browser.net.URLLoaderDataFormat.BINARY);
	ApplicationMain.urlLoaders.set("assets/bins/binary.bin",urlLoader);
	ApplicationMain.total++;
	var loader = new browser.display.Loader();
	ApplicationMain.loaders.set("assets/img/Vikasa_output.png",loader);
	ApplicationMain.total++;
	var loader1 = new browser.display.Loader();
	ApplicationMain.loaders.set("img/Vikasa_output.png",loader1);
	ApplicationMain.total++;
	var urlLoader1 = new browser.net.URLLoader();
	urlLoader1.set_dataFormat(browser.net.URLLoaderDataFormat.BINARY);
	ApplicationMain.urlLoaders.set("bins/binary.bin",urlLoader1);
	ApplicationMain.total++;
	...
	if(ApplicationMain.total == 0) ApplicationMain.begin(); else {
		...
		while( $it0.hasNext() ) {
			...
			loader2.load(new browser.net.URLRequest(path));
		}
		...
		while( $it1.hasNext() ) {
			...
			urlLoader2.load(new browser.net.URLRequest(path));
		}
	}
}
...
ApplicationMain.preloader_onComplete = function(event) {
	...
	} else Reflect.field(com.sync.bones2.Main,"main").apply(com.sync.bones2.Main,[]);
}
var browser = {}
browser.events = {}
...
browser.display.DisplayObject.prototype = $extend(browser.events.EventDispatcher.prototype,{
	nmeSrUpdateDivs: function() {
		...
		var pnt = this.parent.localToGlobal(new browser.geom.Point(this.get_x(),this.get_y()));
	...
	,nmeUnifyChildrenWithDOM: function(lastMoveObj) {
		...
		if(gfx != null && lastMoveObj != null && this != lastMoveObj) {
			...
			if(ogfx != null) browser.Lib.nmeSetSurfaceZIndexAfter(this.nmeScrollRect == null?gfx.nmeSurface:this._srWindow,lastMoveObj.nmeScrollRect == null?ogfx.nmeSurface:lastMoveObj == this.parent?ogfx.nmeSurface:lastMoveObj._srWindow);
		}
...
browser.display.DisplayObjectContainer.prototype = $extend(browser.display.InteractiveObject.prototype,{
	set_scrollRect: function(inValue) {
		inValue = browser.display.InteractiveObject.prototype.set_scrollRect.call(this,inValue);
	...
	,nmeUnifyChildrenWithDOM: function(lastMoveObj) {
		var obj = browser.display.InteractiveObject.prototype.nmeUnifyChildrenWithDOM.call(this,lastMoveObj);
		var _g = 0, _g1 = this.nmeChildren;
		while(_g < _g1.length) {
			var child = _g1[_g];
			++_g;
	...
	,nmeSwapSurface: function(c1,c2) {
		...
		if(gfx1 != null && gfx2 != null) browser.Lib.nmeSwapSurface(this.nmeChildren[c1].nmeScrollRect == null?gfx1.nmeSurface:this.nmeChildren[c1].nmeGetSrWindow(),this.nmeChildren[c2].nmeScrollRect == null?gfx2.nmeSurface:this.nmeChildren[c2].nmeGetSrWindow());
	}
	,nmeRender: function(inMask,clipRect) {
...
com.sync = {}
com.sync.bones2 = {}
com.sync.bones2.Main = function() {
	browser.display.Sprite.call(this);
	this.addEventListener(browser.events.Event.ADDED_TO_STAGE,$bind(this,this.added));
};
$hxClasses["com.sync.bones2.Main"] = com.sync.bones2.Main;
com.sync.bones2.Main.__name__ = ["com","sync","bones2","Main"];
com.sync.bones2.Main.main = function() {
	...
	nme.Lib.get_current().addChild(new com.sync.bones2.Main());
}
com.sync.bones2.Main.__super__ = browser.display.Sprite;
com.sync.bones2.Main.prototype = $extend(browser.display.Sprite.prototype,{
	added: function(e) {
	...
	,init: function() {
		...
		this.inited = true;
		this.img = nme.Assets.getBitmapData("img/Vikasa_output.png");
		this.ba = nme.Assets.getBytes("bins/binary.bin");
		var parser = new com.sync.bones2.parser.ParseBytes(this.ba,this.img);
		var _g1 = 0, _g = Lambda.count(parser.bonesManager.bones);
		while(_g1 < _g) {
			var index = _g1++;
			parser.bonesManager.bones.get(index).addToStage(0,0,this.get_stage());
		}
	}
	,resize: function(e) {
		if(!this.inited) this.init();
	}
	,__class__: com.sync.bones2.Main
});
var DocumentClass = function() {
	com.sync.bones2.Main.call(this);
};
$hxClasses["DocumentClass"] = DocumentClass;
DocumentClass.__name__ = ["DocumentClass"];
DocumentClass.__super__ = com.sync.bones2.Main;
DocumentClass.prototype = $extend(com.sync.bones2.Main.prototype,{
	get_stage: function() {
...
browser.Lib.__properties__ = {get_current:"get_current"}
...
browser.Lib.nmeAppendSurface = function(surface,before,after) {
	if(browser.Lib.mMe.__scr != null) {
		...
		if(before != null) before.parentNode.insertBefore(surface,before); else if(after != null && after.nextSibling != null) after.parentNode.insertBefore(surface,after.nextSibling); else browser.Lib.mMe.__scr.appendChild(surface);
	}
}
...
browser.Lib.nmeBootstrap = function() {
	if(browser.Lib.mMe == null) {
		...
		if(target == null) {
			haxe.Log.trace("Error: Cannot find element ID \"" + "haxe:jeash" + "\"",{ fileName : "Lib.hx", lineNumber : 187, className : "browser.Lib", methodName : "nmeBootstrap"});
...
browser.Lib.nmeCreateSurfaceAnimationCSS = function(surface,data,template,templateFunc,fps,discrete,infinite) {
	...
	if(surface.getAttribute("data-nme-anim") != null) style = js.Browser.document.getElementById(surface.getAttribute("data-nme-anim")); else {
		style = browser.Lib.mMe.__scr.appendChild(js.Browser.document.createElement("style"));
...
browser.Lib.nmeGetHeight = function() {
	var tgt = browser.Lib.mMe != null?browser.Lib.mMe.__scr:js.Browser.document.getElementById("haxe:jeash");
...
browser.Lib.nmeIsOnStage = function(surface) {
	var p = surface;
	while(p != null && p != browser.Lib.mMe.__scr) p = p.parentNode;
	return p == browser.Lib.mMe.__scr;
}
...
browser.Lib.Run = function(tgt,width,height) {
	...
	if(Reflect.hasField(js.Browser.window,"on" + "devicemotion")) js.Browser.window.addEventListener("devicemotion",($_=browser.Lib.nmeGetStage(),$bind($_,$_.nmeQueueStageEvent)),true);
	if(Reflect.hasField(js.Browser.window,"on" + "orientationchange")) js.Browser.window.addEventListener("orientationchange",($_=browser.Lib.nmeGetStage(),$bind($_,$_.nmeQueueStageEvent)),true);
	var _g = 0, _g1 = browser.Lib.HTML_WINDOW_EVENT_TYPES;
	while(_g < _g1.length) {
		var type = _g1[_g];
		++_g;
		js.Browser.window.addEventListener(type,($_=browser.Lib.nmeGetStage(),$bind($_,$_.nmeQueueStageEvent)),false);
	}
...
browser.display.Bitmap = function(inBitmapData,inPixelSnapping,inSmoothing) {
	...
	if(inBitmapData != null) {
		...
		if(this.bitmapData.nmeReferenceCount == 1) this.nmeGraphics = new browser.display.Graphics(this.bitmapData._nmeTextureBuffer);
	}
	if(this.nmeGraphics == null) this.nmeGraphics = new browser.display.Graphics();
	...
	set_bitmapData: function(inBitmapData) {
		if(inBitmapData != this.bitmapData) {
			if(this.bitmapData != null) {
				this.bitmapData.nmeReferenceCount--;
				if(this.nmeGraphics.nmeSurface == this.bitmapData._nmeTextureBuffer) browser.Lib.nmeSetSurfaceOpacity(this.bitmapData._nmeTextureBuffer,0);
			}
...
browser.display.BitmapData.prototype = {
	...
	,threshold: function(sourceBitmapData,sourceRect,destPoint,operation,threshold,color,mask,copySource) {
		...
		haxe.Log.trace("BitmapData.threshold not implemented",{ fileName : "BitmapData.hx", lineNumber : 1353, className : "browser.display.BitmapData", methodName : "threshold"});
		return 0;
	}
	...
	,nmeLoadFromBytes: function(bytes,inRawAlpha,onload) {
		...
		var drawImage = function(_) {
			...
			_g.rect = new browser.geom.Rectangle(0,0,canvas.width,canvas.height);
	...
	,nmeFloodFill: function(x,y,color,replaceColor) {
		if(this.getPixel32(x,y) == replaceColor) {
			this.setPixel32(x,y,color);
			this.nmeFloodFill(x + 1,y,color,replaceColor);
			this.nmeFloodFill(x + 1,y + 1,color,replaceColor);
			this.nmeFloodFill(x + 1,y - 1,color,replaceColor);
			this.nmeFloodFill(x - 1,y,color,replaceColor);
			this.nmeFloodFill(x - 1,y + 1,color,replaceColor);
			this.nmeFloodFill(x - 1,y - 1,color,replaceColor);
			this.nmeFloodFill(x,y + 1,color,replaceColor);
			this.nmeFloodFill(x,y - 1,color,replaceColor);
		}
	}
	...
	,floodFill: function(x,y,color) {
		var queue = new Array();
		queue.push(new browser.geom.Point(x,y));
		...
		while(queue.length > 0) {
			...
			if(this.getPixel32(x1,y1) == old) {
				...
				if(!search[x1 + 1][y1]) queue.push(new browser.geom.Point(x1 + 1,y1));
				if(!search[x1][y1 + 1]) queue.push(new browser.geom.Point(x1,y1 + 1));
				if(x1 > 0 && !search[x1 - 1][y1]) queue.push(new browser.geom.Point(x1 - 1,y1));
				if(y1 > 0 && !search[x1][y1 - 1]) queue.push(new browser.geom.Point(x1,y1 - 1));
			}
		}
	}
	...
	,applyFilter: function(sourceBitmapData,sourceRect,destPoint,filter) {
		haxe.Log.trace("BitmapData.applyFilter not implemented",{ fileName : "BitmapData.hx", lineNumber : 98, className : "browser.display.BitmapData", methodName : "applyFilter"});
	}
...
browser.display.BlendMode = $hxClasses["browser.display.BlendMode"] = { __ename__ : true, __constructs__ : ["ADD","ALPHA","DARKEN","DIFFERENCE","ERASE","HARDLIGHT","INVERT","LAYER","LIGHTEN","MULTIPLY","NORMAL","OVERLAY","SCREEN","SUBTRACT"] }
...
browser.display.GradientType = $hxClasses["browser.display.GradientType"] = { __ename__ : true, __constructs__ : ["RADIAL","LINEAR"] }
...
browser.display.Graphics.prototype = {
	...
	,nmeDrawTiles: function(sheet,tileData,flags) {
		...
		if(ctx != null) while(index < totalCount) {
			...
			if(rect != null && center != null) {
				...
				if(useTransform) ctx.transform(tileData[index + transformIndex],tileData[index + transformIndex + 2],tileData[index + transformIndex + 1],tileData[index + transformIndex + 3],0,0);
	...
	,drawGraphicsData: function(points) {
		var $it0 = ((function(_e) {
			return function() {
				return $iterator(nme._Vector.Vector_Impl_)(_e);
			};
		})(points))();
		while( $it0.hasNext() ) {
			var data = $it0.next();
			...
			case browser.display.GraphicsDataType.PATH:
				var path = data;
				var j = 0;
				var _g1 = 0, _g = nme._Vector.Vector_Impl_.get_length(path.commands);
				while(_g1 < _g) {
					var i = _g1++;
...
browser.display.GraphicsPath.prototype = {
	moveTo: function(x,y) {
		if(this.commands != null && this.data != null) {
			nme._Vector.Vector_Impl_.push(this.commands,1);
			nme._Vector.Vector_Impl_.push(this.data,x);
			nme._Vector.Vector_Impl_.push(this.data,y);
		}
	}
	,lineTo: function(x,y) {
		if(this.commands != null && this.data != null) {
			nme._Vector.Vector_Impl_.push(this.commands,2);
			nme._Vector.Vector_Impl_.push(this.data,x);
			nme._Vector.Vector_Impl_.push(this.data,y);
		}
	}
	,curveTo: function(controlX,controlY,anchorX,anchorY) {
		if(this.commands != null && this.data != null) {
			nme._Vector.Vector_Impl_.push(this.commands,3);
			nme._Vector.Vector_Impl_.push(this.data,anchorX);
			nme._Vector.Vector_Impl_.push(this.data,anchorY);
			nme._Vector.Vector_Impl_.push(this.data,controlX);
			nme._Vector.Vector_Impl_.push(this.data,controlY);
		}
	}
...
browser.display.GraphicsPathWinding = $hxClasses["browser.display.GraphicsPathWinding"] = { __ename__ : true, __constructs__ : ["EVEN_ODD","NON_ZERO"] }
...
browser.display.GraphicsDataType = $hxClasses["browser.display.GraphicsDataType"] = { __ename__ : true, __constructs__ : ["STROKE","SOLID","GRADIENT","PATH"] }
...
browser.display.GraphicsFillType = $hxClasses["browser.display.GraphicsFillType"] = { __ename__ : true, __constructs__ : ["SOLID_FILL","GRADIENT_FILL"] }
...
browser.display.InterpolationMethod = $hxClasses["browser.display.InterpolationMethod"] = { __ename__ : true, __constructs__ : ["RGB","LINEAR_RGB"] }
...
browser.display.Loader.__super__ = browser.display.Sprite;
browser.display.Loader.prototype = $extend(browser.display.Sprite.prototype,{
	...
	,validateBounds: function() {
		if(this.get__boundsInvalid()) {
			browser.display.Sprite.prototype.validateBounds.call(this);
			if(this.mImage != null) {
	...
	,loadBytes: function(buffer) {
		...
		try {
			...
			haxe.Log.trace("Error " + Std.string(e),{ fileName : "Loader.hx", lineNumber : 116, className : "browser.display.Loader", methodName : "loadBytes"});
	...
	,load: function(request,context) {
		...
		try {
			...
			haxe.Log.trace("Error " + Std.string(e),{ fileName : "Loader.hx", lineNumber : 79, className : "browser.display.Loader", methodName : "load"});
			var evt = new browser.events.IOErrorEvent(browser.events.IOErrorEvent.IO_ERROR);
			evt.currentTarget = this;
			this.contentLoaderInfo.dispatchEvent(evt);
			return;
...
browser.display.SpreadMethod = $hxClasses["browser.display.SpreadMethod"] = { __ename__ : true, __constructs__ : ["REPEAT","REFLECT","PAD"] }
...
browser.events.SecurityErrorEvent = function(type,bubbles,cancelable,text) {
	if(text == null) text = "";
	if(cancelable == null) cancelable = false;
	if(bubbles == null) bubbles = false;
	browser.events.ErrorEvent.call(this,type,bubbles,cancelable);
	this.text = text;
};
$hxClasses["browser.events.SecurityErrorEvent"] = browser.events.SecurityErrorEvent;
browser.events.SecurityErrorEvent.__name__ = ["browser","events","SecurityErrorEvent"];
browser.events.SecurityErrorEvent.__super__ = browser.events.ErrorEvent;
browser.events.SecurityErrorEvent.prototype = $extend(browser.events.ErrorEvent.prototype,{
	__class__: browser.events.SecurityErrorEvent
});
...
browser.media.Sound.prototype = $extend(browser.events.EventDispatcher.prototype,{
	...
	,nmeOnSoundLoadError: function(evt) {
		this.nmeRemoveEventListeners();
		haxe.Log.trace("Error loading sound '" + this.nmeStreamUrl + "'",{ fileName : "Sound.hx", lineNumber : 213, className : "browser.media.Sound", methodName : "nmeOnSoundLoadError"});
	...
	,nmeLoad: function(stream,context,mime) {
		...
		if(mime == null || !browser.media.Sound.nmeCanPlayMime(mime)) haxe.Log.trace("Warning: '" + stream.url + "' with type '" + mime + "' may not play on this browser.",{ fileName : "Sound.hx", lineNumber : 127, className : "browser.media.Sound", methodName : "nmeLoad"});
		...
		try {
			...
			haxe.Log.trace("Warning: Could not preload '" + stream.url + "'",{ fileName : "Sound.hx", lineNumber : 142, className : "browser.media.Sound", methodName : "nmeLoad"});
		}
	}
...
browser.media.SoundChannel.prototype = $extend(browser.events.EventDispatcher.prototype,{
	...
	,__onStalled: function(evt) {
		haxe.Log.trace("sound stalled",{ fileName : "SoundChannel.hx", lineNumber : 171, className : "browser.media.SoundChannel", methodName : "__onStalled"});
	...
	,__onProgress: function(evt) {
		haxe.Log.trace("sound progress: " + Std.string(evt),{ fileName : "SoundChannel.hx", lineNumber : 117, className : "browser.media.SoundChannel", methodName : "__onProgress"});
	}
	,stop: function() {
...
browser.net.URLLoader = function(request) {
	...
	this.bytesLoaded = 0;
	this.bytesTotal = 0;
	this.set_dataFormat(browser.net.URLLoaderDataFormat.TEXT);
...
browser.net.URLLoader.prototype = $extend(browser.events.EventDispatcher.prototype,{
	...
	,onSecurityError: function(msg) {
		var evt = new browser.events.SecurityErrorEvent(browser.events.SecurityErrorEvent.SECURITY_ERROR);
		evt.text = msg;
		evt.currentTarget = this;
		this.dispatchEvent(evt);
	}
	...
	,set_dataFormat: function(inputVal) {
		if(inputVal == browser.net.URLLoaderDataFormat.BINARY && !Reflect.hasField(js.Browser.window,"ArrayBuffer")) this.dataFormat = browser.net.URLLoaderDataFormat.TEXT; else this.dataFormat = inputVal;
...
browser.net.URLRequest.prototype = {
	formatRequestHeaders: function() {
		...
		if(res == null) res = [];
		if(this.method == browser.net.URLRequestMethod.GET || this.data == null) return res;
		if(js.Boot.__instanceof(this.data,String) || js.Boot.__instanceof(this.data,browser.utils.ByteArray)) {
			res = res.slice();
			res.push(new browser.net.URLRequestHeader("Content-Type",this.contentType != null?this.contentType:"application/x-www-form-urlencoded"));
		}
		return res;
	}
...
browser.text.Font.nmeOfResource = function(name) {
	...
	if(data == null) haxe.Log.trace("Resource data for string '" + name + "' not found.",{ fileName : "Font.hx", lineNumber : 113, className : "browser.text.Font", methodName : "nmeOfResource"}); else browser.text.Font.nmeFontData[name] = data;
}
...
browser.text.FontStyle = $hxClasses["browser.text.FontStyle"] = { __ename__ : true, __constructs__ : ["REGULAR","ITALIC","BOLD_ITALIC","BOLD"] }
...
browser.text.FontType = $hxClasses["browser.text.FontType"] = { __ename__ : true, __constructs__ : ["EMBEDDED","DEVICE"] }
...
browser.utils.ByteArray.fromBytes = function(inBytes) {
	var result = new browser.utils.ByteArray();
...
com.sync.bones2.parser = {}
com.sync.bones2.parser.BonesAlias = function(byteArray) {
	this._ba = byteArray;
	byteArray = null;
	this._names = new haxe.ds.IntMap();
	this._parse();
};
$hxClasses["com.sync.bones2.parser.BonesAlias"] = com.sync.bones2.parser.BonesAlias;
com.sync.bones2.parser.BonesAlias.__name__ = ["com","sync","bones2","parser","BonesAlias"];
com.sync.bones2.parser.BonesAlias.prototype = {
	_parse: function() {
		this._ba.position = 0;
		while(this._ba.position < this._ba.length) {
			var name = this._ba.readUTF();
			var index = this._ba.readInt();
	...
	,__class__: com.sync.bones2.parser.BonesAlias
}
com.sync.bones2.parser.BonesArmature = function(byteArray) {
	this._ba = byteArray;
	this._startPosition = new Array();
	this.parse();
};
$hxClasses["com.sync.bones2.parser.BonesArmature"] = com.sync.bones2.parser.BonesArmature;
com.sync.bones2.parser.BonesArmature.__name__ = ["com","sync","bones2","parser","BonesArmature"];
com.sync.bones2.parser.BonesArmature.prototype = {
	parse: function() {
		this._ba.position = 0;
		this._frameRate = this._ba.readInt();
		this._name = this._ba.readUTF();
		var armatureLength = this._ba.readInt();
		var armatureName = this._ba.readUTF();
		haxe.Log.trace("Name: " + this._name + " FrameRate: " + this._frameRate,{ fileName : "BonesArmature.hx", lineNumber : 36, className : "com.sync.bones2.parser.BonesArmature", methodName : "parse"});
		while(this._ba.position < this._ba.length) {
			var X = this._ba.readInt() / 1000;
			var Y = this._ba.readInt() / 1000;
			var skewX = this._ba.readInt() / 1000;
			var skewY = this._ba.readInt() / 1000;
			var scaleX = this._ba.readInt() / 1000;
			var scaleY = this._ba.readInt() / 1000;
			var pivotX = this._ba.readInt() / 1000;
			var pivotY = this._ba.readInt() / 1000;
			var z = this._ba.readInt();
			var atlas = this._ba.readInt();
			this._startPosition[atlas] = [X,Y,skewX,skewY,scaleX,scaleY,pivotX,pivotY,z];
		}
		haxe.Log.trace("Armature: " + armatureName,{ fileName : "BonesArmature.hx", lineNumber : 59, className : "com.sync.bones2.parser.BonesArmature", methodName : "parse"});
		haxe.Log.trace(this._startPosition,{ fileName : "BonesArmature.hx", lineNumber : 60, className : "com.sync.bones2.parser.BonesArmature", methodName : "parse"});
	}
	,getBoneParams: function(atlasId) {
		if(atlasId < 0 || atlasId >= this._startPosition.length) return null;
		return this._startPosition[atlasId];
	}
	,__class__: com.sync.bones2.parser.BonesArmature
}
com.sync.bones2.parser.BonesAtlas = function(byteArray) {
	this._ba = byteArray;
	byteArray = null;
	this._coords = new haxe.ds.IntMap();
	this._parse();
};
$hxClasses["com.sync.bones2.parser.BonesAtlas"] = com.sync.bones2.parser.BonesAtlas;
com.sync.bones2.parser.BonesAtlas.__name__ = ["com","sync","bones2","parser","BonesAtlas"];
com.sync.bones2.parser.BonesAtlas.prototype = {
	_parse: function() {
		haxe.Log.trace(this._ba.length,{ fileName : "BonesAtlas.hx", lineNumber : 33, className : "com.sync.bones2.parser.BonesAtlas", methodName : "_parse"});
		this._ba.position = 0;
		while(this._ba.position < this._ba.length) {
			var item = new haxe.ds.IntMap();
			var index = this._ba.readInt();
			var v = this._ba.readInt();
			item.set(com.sync.bones2.parser.BonesAtlas.X,v);
			v;
			var v = this._ba.readInt();
			item.set(com.sync.bones2.parser.BonesAtlas.Y,v);
			v;
			var v = this._ba.readInt();
			item.set(com.sync.bones2.parser.BonesAtlas.W,v);
			v;
			var v = this._ba.readInt();
			item.set(com.sync.bones2.parser.BonesAtlas.H,v);
			v;
			this._coords.set(index,item);
		}
	}
	,getBoneAtlas: function(boneIndex) {
		if(Lambda.count(this._coords) < boneIndex) return null;
		return this._coords.get(boneIndex);
	}
	,__class__: com.sync.bones2.parser.BonesAtlas
}
com.sync.bones2.parser.ParseBytes = function(byteArray,texture) {
	this._ba = byteArray;
	this._bitmap = texture;
	this._parse();
};
$hxClasses["com.sync.bones2.parser.ParseBytes"] = com.sync.bones2.parser.ParseBytes;
com.sync.bones2.parser.ParseBytes.__name__ = ["com","sync","bones2","parser","ParseBytes"];
com.sync.bones2.parser.ParseBytes.prototype = {
	_parse: function() {
		this._ba.position = 0;
		var alias = new browser.utils.ByteArray();
		var atlas = new browser.utils.ByteArray();
		var armatures = new browser.utils.ByteArray();
		var aliasLength = this._ba.readInt();
		alias.writeBytes(this._ba,4,aliasLength);
		this._ba.position = aliasLength + 4;
		var atlasLength = this._ba.readInt();
		atlas.writeBytes(this._ba,8 + aliasLength,atlasLength);
		this._ba.position = 8 + aliasLength + atlasLength;
		var armaturesLength = this._ba.readInt();
		armatures.writeBytes(this._ba,this._ba.position,armaturesLength);
		this.alias = new com.sync.bones2.parser.BonesAlias(alias);
		this.atlas = new com.sync.bones2.parser.BonesAtlas(atlas);
		this.armatures = new com.sync.bones2.parser.BonesArmature(armatures);
		this.bonesManager = new com.sync.bones2.visual.BonesManager(this.alias,this.atlas,this._bitmap,this.armatures);
	}
	,__class__: com.sync.bones2.parser.ParseBytes
}
com.sync.bones2.visual = {}
com.sync.bones2.visual.Bone = function(index,texture,coords) {
	if(coords == null) return;
	var rect = new browser.geom.Rectangle(coords.get(com.sync.bones2.parser.BonesAtlas.X),coords.get(com.sync.bones2.parser.BonesAtlas.Y),coords.get(com.sync.bones2.parser.BonesAtlas.W),coords.get(com.sync.bones2.parser.BonesAtlas.H));
	var point = new browser.geom.Point();
	this._bitmap = new browser.display.BitmapData(rect.width | 0,rect.height | 0);
	this._bitmap.copyPixels(texture,rect,point);
	this._bmp = new browser.display.Bitmap(this._bitmap);
	point = null;
	rect = null;
};
$hxClasses["com.sync.bones2.visual.Bone"] = com.sync.bones2.visual.Bone;
com.sync.bones2.visual.Bone.__name__ = ["com","sync","bones2","visual","Bone"];
com.sync.bones2.visual.Bone.prototype = {
	addToStage: function(x,y,stage) {
		stage.addChild(this._bmp);
	}
	,setupMatrix: function(boneData) {
		this._bmp.set_x(boneData[com.sync.bones2.visual.Bone.X] - boneData[com.sync.bones2.visual.Bone.pivotX]);
		this._bmp.set_y(boneData[com.sync.bones2.visual.Bone.Y] - boneData[com.sync.bones2.visual.Bone.pivotY]);
		this._bmp.set_rotation(boneData[com.sync.bones2.visual.Bone.skewY]);
		this._bmp.set_scaleX(boneData[com.sync.bones2.visual.Bone.scaleX]);
		this._bmp.set_scaleY(boneData[com.sync.bones2.visual.Bone.scaleY]);
		var _g = this._bmp;
		_g.set_x(_g.get_x() + 200);
		var _g = this._bmp;
		_g.set_y(_g.get_y() + 220);
	}
	,__class__: com.sync.bones2.visual.Bone
}
com.sync.bones2.visual.BonesManager = function(alias,atlas,texture,armatures) {
	this._alias = alias;
	this._atlas = atlas;
	this._bitmap = texture;
...
$hxClasses["com.sync.bones2.visual.BonesManager"] = com.sync.bones2.visual.BonesManager;
com.sync.bones2.visual.BonesManager.__name__ = ["com","sync","bones2","visual","BonesManager"];
com.sync.bones2.visual.BonesManager.prototype = {
	createBones: function() {
		var names = this._alias.getNames();
		var itemsCount = Lambda.count(names);
		var result = new haxe.ds.IntMap();
		var _g = 0;
		while(_g < itemsCount) {
			var index = _g++;
			var coords = this._atlas.getBoneAtlas(index);
			var bone = new com.sync.bones2.visual.Bone(index,this._bitmap,coords);
			bone.setupMatrix(this._armatures.getBoneParams(index));
			haxe.Log.trace("Name: " + names.get(index) + Std.string(this._armatures.getBoneParams(index)),{ fileName : "BonesManager.hx", lineNumber : 36, className : "com.sync.bones2.visual.BonesManager", methodName : "createBones"});
			result.set(itemsCount - index - 1,bone);
			bone;
		}
		this.bones = result;
	}
	,__class__: com.sync.bones2.visual.BonesManager
}
var format = {}
format.display = {}
...
nme.AssetData.initialize = function() {
	if(!nme.AssetData.initialized) {
		nme.AssetData.path.set("assets/bins/binary.bin","assets/bins/binary.bin");
		var value = Reflect.field(nme.AssetType,"binary".toUpperCase());
		nme.AssetData.type.set("assets/bins/binary.bin",value);
		nme.AssetData.path.set("assets/img/Vikasa_output.png","assets/img/Vikasa_output.png");
		var value = Reflect.field(nme.AssetType,"image".toUpperCase());
		nme.AssetData.type.set("assets/img/Vikasa_output.png",value);
		nme.AssetData.path.set("img/Vikasa_output.png","img/Vikasa_output.png");
		var value = Reflect.field(nme.AssetType,"image".toUpperCase());
		nme.AssetData.type.set("img/Vikasa_output.png",value);
		nme.AssetData.path.set("bins/binary.bin","bins/binary.bin");
		var value = Reflect.field(nme.AssetType,"binary".toUpperCase());
		nme.AssetData.type.set("bins/binary.bin",value);
...
nme.Assets.__properties__ = {get_type:"get_type",get_path:"get_path",get_library:"get_library",get_id:"get_id"}
nme.Assets.initialize = function() {
	if(!nme.Assets.initialized) {
		nme.AssetData.initialize();
		nme.Assets.initialized = true;
	}
}
nme.Assets.getBitmapData = function(id,useCache) {
	if(useCache == null) useCache = true;
	nme.Assets.initialize();
	if(nme.AssetData.type.exists(id) && nme.AssetData.type.get(id) == nme.AssetType.IMAGE) {
		if(useCache && nme.Assets.cachedBitmapData.exists(id)) return nme.Assets.cachedBitmapData.get(id); else {
			var data = (js.Boot.__cast(ApplicationMain.loaders.get(nme.AssetData.path.get(id)).contentLoaderInfo.content , browser.display.Bitmap)).bitmapData;
			if(useCache) nme.Assets.cachedBitmapData.set(id,data);
			return data;
		}
		...
		} else haxe.Log.trace("[nme.Assets] There is no asset library named \"" + libraryName + "\"",{ fileName : "Assets.hx", lineNumber : 174, className : "nme.Assets", methodName : "getBitmapData"});
	} else haxe.Log.trace("[nme.Assets] There is no BitmapData asset with an ID of \"" + id + "\"",{ fileName : "Assets.hx", lineNumber : 180, className : "nme.Assets", methodName : "getBitmapData"});
	return null;
}
nme.Assets.getBytes = function(id) {
	nme.Assets.initialize();
	if(nme.AssetData.type.exists(id)) {
		...
		if(js.Boot.__instanceof(data,String)) {
			var bytes1 = new browser.utils.ByteArray();
			bytes1.writeUTFBytes(data);
		} else if(js.Boot.__instanceof(data,browser.utils.ByteArray)) bytes = data; else bytes = null;
		if(bytes != null) {
			bytes.position = 0;
			return bytes;
		} else return null;
	} else haxe.Log.trace("[nme.Assets] There is no String or ByteArray asset with an ID of \"" + id + "\"",{ fileName : "Assets.hx", lineNumber : 237, className : "nme.Assets", methodName : "getBytes"});
	return null;
}
nme.Assets.getFont = function(id) {
	nme.Assets.initialize();
	if(nme.AssetData.type.exists(id) && nme.AssetData.type.get(id) == nme.AssetType.FONT) return js.Boot.__cast(Type.createInstance(nme.AssetData.className.get(id),[]) , browser.text.Font); else haxe.Log.trace("[nme.Assets] There is no Font asset with an ID of \"" + id + "\"",{ fileName : "Assets.hx", lineNumber : 274, className : "nme.Assets", methodName : "getFont"});
	return null;
}
nme.Assets.getMovieClip = function(id) {
	...
	var symbolName = HxOverrides.substr(id,id.indexOf(":") + 1,null);
	if(nme.AssetData.library.exists(libraryName)) {
	} else haxe.Log.trace("[nme.Assets] There is no asset library named \"" + libraryName + "\"",{ fileName : "Assets.hx", lineNumber : 346, className : "nme.Assets", methodName : "getMovieClip"});
	return null;
}
nme.Assets.getSound = function(id) {
	nme.Assets.initialize();
	if(nme.AssetData.type.exists(id)) {
		var type = nme.AssetData.type.get(id);
		if(type == nme.AssetType.SOUND || type == nme.AssetType.MUSIC) return new browser.media.Sound(new browser.net.URLRequest(nme.AssetData.path.get(id)));
	}
	haxe.Log.trace("[nme.Assets] There is no Sound asset with an ID of \"" + id + "\"",{ fileName : "Assets.hx", lineNumber : 393, className : "nme.Assets", methodName : "getSound"});
	return null;
}
nme.Assets.getText = function(id) {
	var bytes = nme.Assets.getBytes(id);
	if(bytes == null) return null; else return bytes.readUTFBytes(bytes.length);
}
nme.Assets.resolveClass = function(name) {
	name = StringTools.replace(name,"native.","browser.");
	return Type.resolveClass(name);
}
nme.Assets.resolveEnum = function(name) {
	name = StringTools.replace(name,"native.","browser.");
	return Type.resolveEnum(name);
}
nme.Assets.get_id = function() {
	nme.Assets.initialize();
	var ids = [];
...
nme.Assets.get_library = function() {
	nme.Assets.initialize();
	return nme.AssetData.library;
}
nme.Assets.get_path = function() {
	nme.Assets.initialize();
	return nme.AssetData.path;
}
nme.Assets.get_type = function() {
	nme.Assets.initialize();
	return nme.AssetData.type;
}
nme.AssetType = $hxClasses["nme.AssetType"] = { __ename__ : true, __constructs__ : ["BINARY","FONT","IMAGE","MUSIC","SOUND","TEXT"] }
...
nme.LibraryType = $hxClasses["nme.LibraryType"] = { __ename__ : true, __constructs__ : ["SWF","XFL"] }
nme.LibraryType.SWF = ["SWF",0];
nme.LibraryType.SWF.toString = $estr;
nme.LibraryType.SWF.__enum__ = nme.LibraryType;
nme.LibraryType.XFL = ["XFL",1];
nme.LibraryType.XFL.toString = $estr;
nme.LibraryType.XFL.__enum__ = nme.LibraryType;
nme.Lib = function() { }
...
nme.Lib["as"] = function(v,c) {
	return v;
}
nme.Lib.attach = function(name) {
	return new browser.display.MovieClip();
}
nme.Lib.close = function() {
}
nme.Lib.create = function(onLoaded,width,height,frameRate,color,flags,title,icon,stageClass) {
...
nme.Lib.setPackage = function(company,file,packageName,version) {
	nme.Lib._company = company;
	nme.Lib._file = file;
	nme.Lib._packageName = packageName;
	nme.Lib._version = version;
}
nme.Lib.trace = function(arg) {
	browser.Lib.trace(arg);
}
nme.Lib.get_company = function() {
	return nme.Lib._company;
	return "";
}
...
nme.Lib.get_version = function() {
	return nme.Lib._version;
	return "";
}
nme._Vector = {}
nme._Vector.Vector_Impl_ = function() { }
...
nme._Vector.Vector_Impl_.ofArray = function(a) {
	return nme._Vector.Vector_Impl_.concat(nme._Vector.Vector_Impl_._new(),a);
}
...
browser.events.TextEvent.LINK = "link";
browser.events.TextEvent.TEXT_INPUT = "textInput";
...
browser.events.SecurityErrorEvent.SECURITY_ERROR = "securityError";
...
com.sync.bones2.parser.BonesAtlas.X = 0;
com.sync.bones2.parser.BonesAtlas.Y = 1;
com.sync.bones2.parser.BonesAtlas.W = 2;
com.sync.bones2.parser.BonesAtlas.H = 3;
com.sync.bones2.visual.Bone.X = 0;
com.sync.bones2.visual.Bone.Y = 1;
com.sync.bones2.visual.Bone.skewX = 2;
com.sync.bones2.visual.Bone.skewY = 3;
com.sync.bones2.visual.Bone.scaleX = 4;
com.sync.bones2.visual.Bone.scaleY = 5;
com.sync.bones2.visual.Bone.pivotX = 6;
com.sync.bones2.visual.Bone.pivotY = 7;
com.sync.bones2.visual.Bone.z = 8;
...
nme.Assets.cachedBitmapData = new haxe.ds.StringMap();
nme.Assets.initialized = false;
...
//@ sourceMappingURL=Bones2.js.map